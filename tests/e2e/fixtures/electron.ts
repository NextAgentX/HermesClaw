...
async function launchHermesClawElectron(
  homeDir: string,
  userDataDir: string,
...
export const test = base.extend<ElectronFixtures>({
  homeDir: async ({ browserName: _browserName }, provideHomeDir) => {
    const homeDir = await mkdtemp(join(tmpdir(), 'hermesclaw-e2e-home-'));
  ...
  userDataDir: async ({ browserName: _browserName }, provideUserDataDir) => {
    const userDataDir = await mkdtemp(join(tmpdir(), 'hermesclaw-e2e-user-data-'));
    try {
  ...
  launchElectronApp: async ({ homeDir, userDataDir }, provideLauncher) => {
    await provideLauncher(async (options?: LaunchElectronOptions) => await launchHermesClawElectron(homeDir, userDataDir, options));
  },
