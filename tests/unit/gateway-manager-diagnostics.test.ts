...
const connectGatewaySocketMock = vi.fn();
...
describe('GatewayManager diagnostics', () => {
  ...
  beforeEach(() => {
    ...
    connectGatewaySocketMock.mockResolvedValue({
      readyState: 1,
      on: vi.fn(),
      send: vi.fn(),
    });
  ...
  it('uses longer handshake timeouts while startup is still in progress', async () => {
    const { GatewayManager } = await import('@electron/gateway/manager');
    const manager = new GatewayManager();

    (manager as unknown as { deviceIdentity: null }).deviceIdentity = null;
    (manager as unknown as { status: { state: string; port: number } }).status = {
      state: 'starting',
      port: 18789,
    };
    ...
    expect(connectGatewaySocketMock).toHaveBeenCalledWith(
