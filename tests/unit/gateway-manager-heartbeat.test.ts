...
describe('GatewayManager heartbeat recovery', () => {
  ...
  it('restarts after consecutive heartbeat misses reach threshold', async () => {
    ...
    vi.advanceTimersByTime(isWindows ? 400_000 : 120_000);

    expect(ws.ping).toHaveBeenCalledTimes(isWindows ? 5 : 3);
    expect(ws.terminate).not.toHaveBeenCalled();
    expect(restartSpy).toHaveBeenCalledTimes(isWindows ? 0 : 1);
