...
const { testHome, testUserData, mockLoggerWarn, mockLoggerInfo, mockLoggerError } = vi.hoisted(() => {
  ...
  return {
    testHome: `/tmp/hermesclaw-channel-config-${suffix}`,
    testUserData: `/tmp/hermesclaw-channel-config-user-data-${suffix}`,
...
describe('OpenClaw config read resilience', () => {
  beforeEach(async () => {
    vi.resetAllMocks();
    vi.resetModules();
    await rm(testHome, { recursive: true, force: true });
  ...
  it('treats empty config files as a recoverable startup state', async () => {
    const { readOpenClawConfig } = await import('@electron/utils/channel-config');

    const configDir = join(testHome, '.openclaw');
    await mkdir(configDir, { recursive: true });
    await writeFile(join(configDir, 'openclaw.json'), '', 'utf8');

    await expect(readOpenClawConfig()).resolves.toEqual({});
    expect(mockLoggerError).not.toHaveBeenCalled();
    expect(mockLoggerWarn).toHaveBeenCalledWith(
      'OpenClaw config file is empty; treating it as a transient startup state',
      expect.any(String),
    );
  });
});
