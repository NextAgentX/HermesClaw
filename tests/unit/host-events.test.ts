...
describe('host-events', () => {
  ...
  it('maps runtime install progress events to IPC subscriptions', async () => {
    const onMock = vi.mocked(window.electron.ipcRenderer.on);
    const captured: Array<(...args: unknown[]) => void> = [];
    const cleanupSpy = vi.fn();
    onMock.mockImplementation((_, cb: (...args: unknown[]) => void) => {
    ...
    const { subscribeHostEvent } = await import('@/lib/host-events');
    const handler = vi.fn();
    const unsubscribe = subscribeHostEvent('runtime:install:progress', handler);

    expect(onMock).toHaveBeenCalledWith('runtime:install-progress', expect.any(Function));
    expect(createHostEventSourceMock).not.toHaveBeenCalled();

    captured[0]({ installChoice: 'both', activeStepId: 'openclaw' });
    expect(handler).toHaveBeenCalledWith({ installChoice: 'both', activeStepId: 'openclaw' });
  ...
  it('uses SSE fallback only when explicitly enabled', async () => {
    window.localStorage.setItem('hermesclaw:allow-sse-fallback', '1');
    const { subscribeHostEvent } = await import('@/lib/host-events');
    const handler = vi.fn();
