...
const testOpenClawConfigDir = join(tmpdir(), 'hermesclaw-tests', 'diagnostics-routes-openclaw');
...
describe('handleDiagnosticsRoutes', () => {
  beforeEach(() => {
    ...
    readLogFileMock.mockResolvedValue('hermesclaw-log-tail');
  });
  ...
  it('returns diagnostics snapshot with channel view and tailed logs', async () => {
    ...
    expect(payload.hermesclawLogTail).toBe('hermesclaw-log-tail');
